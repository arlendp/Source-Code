...
(function (global, factory) {
   ...
   * @param  {number} step  每个起始处的距离
   * @return {array}       包含每个标记点处的值的数组
   */
  ...
  function tickStep(start, stop, count) {
    var step0 = Math.abs(stop - start) / Math.max(0, count),
        //求step1和step0之间的误差，为什么这么求？？？ step0 >= step1
   ...
   * @param  {function} f      修改属性名
   * @return {object}          构造的map对象
   */
  function map$1(object, f) {
    var map = new Map;

    // 将object对象的可枚举属性复制到map对象中
    if (object instanceof Map) object.each(function(value, key) { map.set(key, value); });

    // 若object是数组，当f函数为null时，将该数组复制给map；当f函数存在时，根据其来修改属性名并复制给map
...
var   tau$1 = 2 * pi$1;
  ...
  Path.prototype = path.prototype = {
    ...
    arcTo: function(x1, y1, x2, y2, r) {
      ...
      // 如果(x0, y0) (x1, y1) (x2, y2)不在一条直线上或者半径为0，连线至(x1, y1)
  ...
  /*
   * d3.arc 绘制圆弧
   * 对于d3中的极坐标系，十二点钟方向对应弧度为0，顺时针方向弧度增加
   * arc中的pad部分是一段固定的线性的距离 padRadius * padAngle，当innerRadius小到一定程度时才会出现间隔不等的情况
   */
    ...
    function arc() {
      ...
      // 如果是一个扇形或者圆环区域
        ...
        var a01 = a0,
            ...
            da0 = da,
            da1 = da,
            // padAngle等分成两部分，分别在startAngle和endAngle中进行调整
            ap = padAngle.apply(this, arguments) / 2,
            // padAngle不近似为0时才计算padRadius
            rp = (ap > epsilon$1) && (padRadius ? +padRadius.apply(this, arguments) : Math.sqrt(r0 * r0 + r1 * r1)),
            // cornerRadius, 最大为(r1 - r0) / 2
        ...
        if (rp > epsilon$1) {
          ...
          // 调整角度时，为了使圆环间的padding相同，内圈需要增加角度，外圈减小
          if ((da0 -= p0 * 2) > epsilon$1) p0 *= (cw ? 1 : -1), a00 += p0, a10 -= p0;
          else da0 = 0, a00 = a10 = (a0 + a1) / 2;
          if ((da1 -= p1 * 2) > epsilon$1) p1 *= (cw ? 1 : -1), a01 += p1, a11 -= p1;
          else da1 = 0, a01 = a11 = (a0 + a1) / 2;
        }
        // (x01, y01)外圈startAngle处坐标，(x10, y10)内圈endAngle处坐标
        ...
        // 移动至外圈startAngle处，画弧线至外圈endAngle处
        else context.moveTo(x01, y01), context.arc(0, 0, r1, a01, a11, !cw);

        // Is there no inner ring, and it’s a circular sector?
        // Or perhaps it’s an annular sector collapsed due to padding?
        // 内半径近似为0或者endAngle与startAngle弧度为0
  ...
  // 默认输入数据为含有两个number元素的数组，分别对应x和y坐标
  function x(p) {
    return p[0];
  }
  ...
  //匹配如rgba(10, 54, 86, 0.8)形式
  var reRgbaInteger = /^rgba\(\s*([-+]?\d+)\s*,\s*([-+]?\d+)\s*,\s*([-+]?\d+)\s*,\s*([-+]?\d+(?:\.\d+)?)\s*\)$/;
  //匹配如rgba(10%, 54%, 86%, 0.8)形式
  var reRgbaPercent = /^rgba\(\s*([-+]?\d+(?:\.\d+)?)%\s*,\s*([-+]?\d+(?:\.\d+)?)%\s*,\s*([-+]?\d+(?:\.\d+)?)%\s*,\s*([-+]?\d+(?:\.\d+)?)\s*\)$/;
  //匹配如hsl(30.5, 20%, 50%)形式
  var reHslPercent = /^hsl\(\s*([-+]?\d+(?:\.\d+)?)\s*,\s*([-+]?\d+(?:\.\d+)?)%\s*,\s*([-+]?\d+(?:\.\d+)?)%\s*\)$/;
  //匹配如hsla(30.5, 20%, 50%, 0.8)形式
  ...
  function color(format) {
    ...
    //parseInt函数将指定进制的数转化为十进制，从字符串第一个字符开始直到不能转化为止。
    return (m = reHex3.exec(format)) ? (m = parseInt(m[1], 16), new Rgb((m >> 8 & 0xf) | (m >> 4 & 0x0f0), (m >> 4 & 0xf) | (m & 0xf0), ((m & 0xf) << 4) | (m & 0xf), 1)) // #12f => (17, 34, 255)
        : (m = reHex6.exec(format)) ? rgbn(parseInt(m[1], 16)) // #1122ff => (17, 34, 255)
  ...
  var interpolateRgb = (function rgbGamma(y) {
    var color = gamma(y);// nogamma，对于rgb的计算是在r、g、b和opacity上分别进行线性插值
  ...
  function interpolate(a, b) {
    ...
    /*
    * 当b为null时，返回true
    * 当b为boolean时，返回一个函数，该函数执行时返回值为b
    * 当b为number时，返回一个函数，该函数可在[a, b]间取值
    * 当b为string时，返回一个函数，该函数根据输入的
     */
  ...
  function parseTypenames(typenames, types) {
    return typenames.trim().split(/^|\s+/).map(function(t) {
      ...
      //如果绑定的事件类型在dispatch初始化时没有传入，则抛出异常
...
var   t1$1 = new Date;
/**
 * [newInterval description]
 * @param  {function} floori  对输入时间进行向下取整，如对hour，12:32:56 => 12:00:00
 * @param  {function} offseti 根据step对时间进行修改
 * @param  {function} count   计算时间间隔
 * @param  {function} field   返回指定时间，如小时数、分钟数。
 * @return {function}
 */
  function newInterval(floori, offseti, count, field) {
    ...
    interval.range = function(start, stop, step) {
      ...
      if (!(start < stop) || !(step > 0)) return range; // 不满足start<stop 或者 step>0条件时
  ...
  var hour = newInterval(function(date) {
    //getTimezoneOffset()以分钟的形式返回当前时间与UTC的时间差，负数表示比UTC超前
    var offset = date.getTimezoneOffset() * durationMinute % durationHour;//得到整点数
  ...
  // 如x=1.23，x.toExponential() = '1.23e+0'，coefficient = '1.23'，return ['123', 0]
  ...
  /*
   *scaleOrdinal
   *将domain存入map对象（通过Map构造函数构造得到）中，该对象的key为domain值，value为该值在domain数组中的索引i加1
   *通过scale(x)方法计算时，若x值在map的key中可以找到，则直接根据该索引i从range中取值range[i % length]；若无法找到，则将x存入domain数组中并同时存入map对象中，返回range[i % length]
   *上述length值为range.length，即若i值大于length则取余数。将所有domain会在计算过程中扩展，但range不会改变，将所有的domain值映射到range中。
   *domain和range均可以为任意类型
   */
  function ordinal(range) {
    ...
    var index = map$1(),
        domain = [],
        unknown = implicit;
    //调用Array.prototype.slice方法，得到数组对象
    ...
    scale.domain = function(_) {
      ...
      //array.push返回新的数组的长度
      //将元素作为键，domain数组长度作为值存到index对象中
      while (++i < n) if (!index.has(key = (d = _[i]) + "")) index.set(key, domain.push(d));
      return scale;
    };

    scale.range = function(_) {
      //将传入值转化为数组并赋值给range，函数返回scale函数
      return arguments.length ? (range = slice$3.call(_), scale) : range.slice();
    };
    //设置unknown值，即当执行scale(x)并从domain中找不到x时，返回该值
  ...
  /*
   *scaleBand
   *band构造时调用了ordinal方法，也是将domain存入map对象中，但不同的是band设置range时只接受两个值，用于设置start和stop，相当于设置了一个结果区间。
   *同时将domain数组的长度作为step的个数，计算每个step的start的值。
   *当计算scale(x)时，若x在map中，则根据索引i得到step的start值即range[i]；否则，返回undefine
   */
  function band() {
    ...
    //每次调用scale下的函数时都会重新调整
    function rescale() {
      //domain数组长度，这里指的是step的个数
      var n = domain().length,
      //根据range$$来判断是否反转
      ...
      //align是计算之后第一个paddingOuter占总的paddingOuter的比值
      start += (stop - start - step * (n - paddingInner)) * align;
      //paddingInner的值是一个比例，是内边距与step的比值
    ...
    scale.padding = function(_) {
      //为paddingInner和paddingOuter设置相同值，若无参数则返回paddingInner值
  ...
  function pointish(scale) {
    var copy = scale.copy;
    //paddingInner固定，只能设置paddingOuter
  ...
  //scalePoint
  //与scaleBand相比相当于bandwith缩小成一个点，paddingInner = 1
  function point$4() {
  ...
  //对解值函数的结果进行修改，当要计算的值小于a时返回0；大于b时返回1；否则正常计算
  ...
  function continuous(deinterpolate$$, reinterpolate) {
    ...
    //插入函数是interpolate，可对各种类型数据进行插值计算，如颜色等
    //x值若小于domain[0]，则计算时则按domain[0]和domain[1]之间的插值函数计算
    function scale(x) {
      return (output || (output = piecewise(domain, range, clamp ? deinterpolateClamp(deinterpolate$$) : deinterpolate$$, interpolate$$)))(+x);
    }
    //invert时传入参数调换range和domain位置（无法对除数字外的其他类型值进行转换）
    scale.invert = function(y) {
      return (input || (input = piecewise(range, domain, deinterpolate, clamp ? reinterpolateClamp(reinterpolate) : reinterpolate)))(+y);
    };
    //有参数时，将参数转换为number类型，并rescale；否则直接返回domain
    scale.domain = function(_) {
  ...
  function pow() {
    ...
    //设置指数，若没设置则默认为1，此时和scaleLinear相同
  ...
  //scaleSqrt等同于scalePow().exponent(0.5)
  function sqrt() {
  ...
  //scaleQuantize
  function quantize$1() {
    //默认的domain是[0.5]，range是[0, 1]，当x >= 0.5时，scale(x) = 1；x < 0.5时，scale(x) = 0
    var x0 = 0,
        x1 = 1,
        n = 1,
        domain = [0.5],
    ...
    //这部分根据range的数量而在x0到x1区间等距离的分段，并存入domain中
    function rescale() {
      var i = -1;
      domain = new Array(n);
      //对于i，domain[i] = x0 + (x1 - x0) / (n + 1) * (i + 1)
...
var   durationWeek$1 = durationDay$1 * 7;
    ...
    function tickInterval(interval, start, stop, step) {
      ...
      if (typeof interval === "number") {
        ...
        if (i === tickIntervals.length) {
          ...
          //选择tickIntervals[i-1]和tickIntervals[i]中更近的值
  ...
  //将字符串转化为包含颜色值的数组，如'125fd65f4862' => ['#125fd6', '#5f4862']
  function colors(s) {
  ...
  /*
   *和continuous类似，但方法更少，如没有range、invert等方法
   *在使用scale方法前要设置interpolator函数，通过sequential或scale.interpolator都可设置
   */
  function sequential(interpolator) {
    ...
    //domain只接受两个number类型（或者可转换为number类型）的值
    scale.domain = function(_) {
  ...
  // 对typenames根据空格来划分成数组，并根据分离后的字符串中的'.'来将该字符串分割为type和name部分
  ...
  /*
   * 先对selection中包含的结点进行复制并在数组中排序，然后再对dom进行排序
   */
  ...
  // 可以设置或获取通过attr和style方法无法得到的属性，如checkbox的checked属性
  ...
  function classedFunction(names, value) {
    // 根据传入arguments进行add和remove方法的选择
    return function() {
  ...
  // 设置元素的innerHTML属性，该属性返回元素内的所有内容。
  ...
  /*
   * Selection构造函数
   * 按照惯例，在以链式形式调用selection的方法时，如果该方法返回当前selection则使用四个空格来缩进；若该方法返回新的selection则使用两个空格缩进，这样便于显示上下文的变化。
   */
  ...
  /*
   * 匹配第一个选中的元素
   * selector可以为字符串类型的选择符，如'p'、'.button'等，也可以是对dom的直接引用，如可以直接select(this)
   */
...
var   keyPrefix$1 = "$";
  ...
  /*
   * 用于chord中带状图形的绘制
   * 当source与target相同时，需要绘制两条曲线；当source与target不同时，要绘制四条曲线，组成一个带状图形。
   */
  function ribbon() {
